...
func (b *Remote) opPlan(stopCtx, cancelCtx context.Context, op *backend.Operation, runningOp *backend.RunningOperation) {
	...
	// TODO SvH: Does a destroy without a config version also work?
	...
	if op.Module != nil && op.Module.Config().Dir != "" {
		...
		cv, err = b.client.ConfigurationVersions.Create(stopCtx, w.ID, options)
		if err != nil {
			if err != context.Canceled {
				runningOp.Err = fmt.Errorf(strings.TrimSpace(fmt.Sprintf(
		...
		err = b.client.ConfigurationVersions.Upload(stopCtx, cv.UploadURL, op.Module.Config().Dir)
		if err != nil {
			if err != context.Canceled {
				runningOp.Err = fmt.Errorf(strings.TrimSpace(fmt.Sprintf(
	...
	r, err := b.client.Runs.Create(stopCtx, options)
	if err != nil {
		if err != context.Canceled {
			runningOp.Err = fmt.Errorf(strings.TrimSpace(fmt.Sprintf(
	...
	for {
		...
		case <-cancelCtx.Done():
			return
		case <-time.After(500 * time.Millisecond):
			p, err := b.client.Plans.Read(stopCtx, r.Plan.ID)
			if err != nil {
				if err != context.Canceled {
					runningOp.Err = fmt.Errorf("Error retrieving plan: %v", err)
				}
				return
			}

			if p.LogReadURL != "" {
				logs, err := b.client.Plans.Logs(stopCtx, p.LogReadURL)
				if err != nil {
					if err != context.Canceled {
						runningOp.Err = fmt.Errorf("Error retrieving logs: %v", err)
					}
					return
				}

				logs = bytes.TrimPrefix(logs, logged)
				if len(logs) > 0 {
					b.CLI.Output(b.Colorize().Color(string(bytes.TrimSuffix(logs, []byte{'\n'}))))
				}
				logged = append(logged, logs...)
			}

			switch p.Status {
			case tfe.PlanCanceled, tfe.PlanErrored, tfe.PlanFinished:
				return
			}
		}
	}
