...
func (h *stopHook) PreApply(addr addrs.AbsResourceInstance, gen states.Generation, priorState, plannedNewState cty.Value) (HookAction, error) {
	return h.hook()
}
...
func (h *stopHook) PreDiff(addr addrs.AbsResourceInstance, priorState, proposedNewState cty.Value) (HookAction, error) {
	return h.hook()
}

func (h *stopHook) PostDiff(addr addrs.AbsResourceInstance, priorState, plannedNewState cty.Value) (HookAction, error) {
	return h.hook()
}
...
func (h *stopHook) PreRefresh(addr addrs.AbsResourceInstance, priorState cty.Value) (HookAction, error) {
	return h.hook()
}

func (h *stopHook) PostRefresh(addr addrs.AbsResourceInstance, priorState cty.Value, newState cty.Value) (HookAction, error) {
	return h.hook()
}
